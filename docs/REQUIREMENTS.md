...
Primary persona: HR Manager of ACME. This take-home models a single operator who maintains employee records, updates current compensation, and reviews pay distribution. No other roles are modeled.
...
- **LLM “ask anything” chat.** A dashboard of defined metrics is the honest, testable answer to how the org pays people.
- **Live FX APIs.** External rates are unstable for tests and offline demos. A seeded rate table is enough for USD-normalized comparison.
...
- Stack (already chosen, not an open decision): FastAPI backend; React with Vite and TypeScript frontend; SQLite. UI: Ant Design, Recharts, and TanStack Query.
